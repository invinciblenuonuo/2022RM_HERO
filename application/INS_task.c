...
/**
  * @brief          不使用RTOS时 陀螺仪初始化函数
  */
int GYRO_SIX_AXIS_Init()
{
		HAL_Delay(10);
    while(BMI088_init())
    {
...
/**
  * @brief          BMI088读取gyro与acel值通过AHRS解算出欧拉角 定时器中断直接调用 非RTOS使用
...
int GYRO_SIX_AXIS()
{
        ...
        AHRS_update(INS_quat, 0.001f, bmi088_real_data.gyro, bmi088_real_data.accel, ist8310_real_data.mag);
        get_angle(INS_quat, INS_angle + INS_YAW_ADDRESS_OFFSET, INS_angle + INS_PITCH_ADDRESS_OFFSET, INS_angle + INS_ROLL_ADDRESS_OFFSET);
				YAW = INS_angle[0]*360/3.1415926f;
				PITCH = INS_angle[1]*360/3.1415926f;
				ROLL = INS_angle[2]*360/3.1415926f;
...
float YAW_CTNU,PITCH_CTNU;
/**
  * @brief          使陀螺仪值连续
  */
int DATA_CONTINUE()
{
	static int YAW_TEMP,PITCH_TEMP;
	...
	else if(YAW - YAW_TEMP < -300)
	{
		i_yaw++;
	}
	
	if(PITCH - PITCH_TEMP > 300)
	{
		i_pitch--;
	}
	else if(PITCH - PITCH_TEMP < -300)
	{
		i_pitch++;
	}
	
	YAW_CTNU = YAW + i_yaw*360;
	PITCH_CTNU = PITCH + i_pitch*360;
